...
class General(commands.Cog, name="general-slash"):
    ...
    async def botinfo(self, interaction: ApplicationCommandInteraction) -> None:
        ...
        """
        embed = disnake.Embed(
            description="Used [nature](https://culturebot.xyz) template",
            color=0x9C84EF
        )
        embed.set_author(
    ...
    async def invite(self, interaction: ApplicationCommandInteraction) -> None:
        ...
        """
        embed = disnake.Embed(
            description=f"Invite me by clicking [here](https://discord.com/api/oauth2/authorize?client_id=979754238571855912&permissions=0&scope=bot%20applications.commands&permissions={self.bot.config['permissions']}).",
            color=0xD75BF4
        )
        try:
    ...
    async def server(self, interaction: ApplicationCommandInteraction) -> None:
        ...
        """
        embed = disnake.Embed(
            description=f"Join the support server for the bot by clicking [here](https://discord.gg/ontop).",
            color=0xD75BF4
        )
        try:
            await interaction.author.send(embed=embed)
